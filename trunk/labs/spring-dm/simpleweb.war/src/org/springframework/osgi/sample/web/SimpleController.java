...
public class SimpleController {
	...
	@RequestMapping("/salute.htm")
	public ModelAndView doit(HttpServletRequest request){
		System.out.println("Hello OSGI MVC");
		ModelAndView mv = new ModelAndView();
		
		
		String input = request.getParameter("secretWord"); 
		if (input == null){
			mv.addObject("saudacao", "say the secret word!");
		} else {
			if(input.startsWith("hal")){
				mv.addObject("saudacao", "De mil passarás a dois mil não chegarás!");
				mv.setViewName("saudacao");
			}else{
				mv.addObject("message", "say the secret word!");
